...
internal static class MigrationsExample
{
    ...
    private static async Task RunMigrationsAsync(IDocumentStore store, SqliteConnection connection, CancellationToken ct)
    {
        ...
        var cityVirtualColumn = new Migration(
            ...
            upSql: $"""
                ALTER TABLE [{customerTable}] ADD COLUMN city TEXT GENERATED ALWAYS AS (json_extract(data, '$.City')) VIRTUAL;
                CREATE INDEX IF NOT EXISTS idx_customer_city ON [{customerTable}](city);
                """,
        ...
        await InsertDocumentAsync(store, connection, customerTable, "c2", new Customer("c2", "Bob Johnson", "bob@example.com", "Los Angeles"), ct);
        await InsertDocumentAsync(store, connection, orderTable, "o1", new Order("o1", "c1", DateTime.UtcNow.AddDays(-2), 150.00m), ct);
        ...
        Console.WriteLine($"Customer exists           => {await introspector.TableExistsAsync(customerTable, ct)}");

        var columns = (await introspector.GetColumnsAsync(customerTable, ct)).ToList();
        Console.WriteLine($"Customer columns          => {string.Join(", ", columns.Select(c => c.Name))}");
        Console.WriteLine($"'city' column exists      => {await introspector.ColumnExistsAsync(customerTable, "city", ct)}");

        var indexes = (await introspector.GetIndexesAsync(customerTable, ct)).ToList();
    ...
    private static async Task InsertDocumentAsync<T>(
        IDocumentStore store, SqliteConnection connection, string table, string id, T data, CancellationToken ct)
    {
        var cmd = connection.CreateCommand();
        cmd.CommandText = $"INSERT OR REPLACE INTO [{table}] (id, data) VALUES (@Id, jsonb(@Data))";
        cmd.Parameters.AddWithValue("@Id", id);
        cmd.Parameters.AddWithValue("@Data", store.SerializeDocument(data));
