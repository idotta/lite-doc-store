...
Three settings used to be applied and silently ignored, measured against real SQLite before being fixed:
...
These are deliberate breaking changes over silence: a store that cannot honour an option now refuses to open rather than pretending. Still open from the audit's P1-11: the missing production PRAGMAs (`mmap_size`, `temp_store`, `journal_size_limit`, `wal_autocheckpoint`, `auto_vacuum`, `PRAGMA optimize` on close) and any `SQLITE_BUSY` retry policy / `BEGIN IMMEDIATE` choice.
